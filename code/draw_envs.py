...
class Paint(object):
    ...
    def setup(self):
        ...
        self.root.bind("<KeyPress-s>", self.save_as_png)
        self.root.bind("<KeyPress-a>", self.reset_key)
        self.c.bind('<Button-1>', self.paint)
        
    def save_as_png(self,event):
        self.c.postscript(file="circles.eps")
        img = Image.open("circles.eps")
        number_of_files = len(os.listdir('../data/images'))
        number_of_files+=1
        img.save(f"../data/images/{number_of_files}.png", "png")
